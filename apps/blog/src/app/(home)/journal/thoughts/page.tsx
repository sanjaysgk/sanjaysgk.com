...
export const metadata: Metadata = {
  title: 'Thoughts',
  description: 'Career reflections, opinions, and life lessons.',
}

export default function ThoughtsPage() {
  const posts = getPostsByCategory('thought')

  return (
    <>
      <Section className='p-4 lg:p-6'>
        <ViewAnimation initial={{ opacity: 0, translateY: -6 }} whileInView={{ opacity: 1, translateY: 0 }}>
          <h1 className='text-3xl font-bold tracking-tighter sm:text-4xl'>Thoughts</h1>
          <p className='mt-2 text-muted-foreground'>Career reflections, opinions, and life lessons.</p>
        </ViewAnimation>
      </Section>
      <Section className='p-4 lg:p-6'>
        <div className='flex flex-col divide-y divide-dashed divide-border'>
          ...
          {posts.length === 0 && <p className='py-8 text-center text-muted-foreground'>No thoughts yet.</p>}
        </div>
      </Section>
    </>
  )
